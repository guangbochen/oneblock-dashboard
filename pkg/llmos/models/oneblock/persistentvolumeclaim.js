...
import { LLMOS, VOLUME_SNAPSHOT } from '@pkg/llmos/types/types';
import { PV, LONGHORN } from '@shell/config/types';
import { DESCRIPTION } from '@shell/config/labels-annotations';
import { ANNOTATIONS } from '@pkg/llmos/config/labels-annotations';
import { findBy } from '@shell/utils/array';
import { get, clone } from '@shell/utils/object';
import { colorForState } from '@shell/plugins/dashboard-store/resource-class';
import IResource from '~/pkg/llmos/models/llmos';
import { PRODUCT_NAME as HARVESTER_PRODUCT } from '../../config/product';

const DEGRADED_ERROR = 'replica scheduling failed';

export default class HciPv extends IResource {
  ...
  get attachVM() {
    const allVMs = this.$rootGetters['llmos/all'](LLMOS.VM);
    const ownedBy =
  ...
  get relatedVolumeSnapshotCounts() {
    const snapshots = this.$rootGetters['llmos/all'](VOLUME_SNAPSHOT);
  ...
  get originalSnapshot() {
    if (this.spec?.dataSource) {
      return this.$rootGetters['llmos/all'](VOLUME_SNAPSHOT).find((V) => V.metadata?.name === this.spec.dataSource.name);
    } else {
      return null;
    }
  }
  ...
  get relatedPV() {
    return this.$rootGetters['llmos/all'](PV).find((pv) => pv.metadata?.name === this.spec?.volumeName);
  }
